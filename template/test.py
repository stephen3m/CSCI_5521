...
class Decision_tree:
    ...
    def generate_tree(self,data,label):
        ...
        if node_entropy < self.min_entropy:
            counts = [0, 0] #count of 0, and count of 1
    ...
    def entropy(self,label):
        ...
        for i in range(len(label)):
            if(label[i] == 0):
                ...
                counts[1] += 1
        counts = np.array(counts) / len(label)
        counts[counts == 0] = 1e-15  # Handle the case where one of the counts is zero
        node_entropy = -1 * np.sum(counts * np.log2(counts))
        return node_entropy
